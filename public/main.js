document.addEventListener('DOMContentLoaded', function(){
  ...
  if (artistForm) {
    ...
    artistForm.addEventListener('submit', function(e){
      ...
      const target = location.pathname === '/add-artist.html' ? '/artists' : location.pathname;
      fetch(target, {
        method: 'post',
        body: data
      })
  ...
  function renderArtistList(artists) {
    ...
    artists.forEach(artist => {
      ...
      ${artist.photos.map(photo => {
        return `<img src="/${photo.path.split('/')[1]}" alt="${photo.name}" width="100" height="100" />`
      }).join('')}
      <a href="artists/${artist._id}/photos/add" class="btn waves-effect waves-light">Add more photos</a>`
      frag.appendChild(li);
    });
